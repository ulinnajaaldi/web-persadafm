...
export const useKabarBeritaFeature = () => {
  ...
  const { data, isLoading, refetch } = useGetKabarBerita({
    value,
    page,
    limit,
  });
  ...
  const { mutate: mutateAdd, isPending: isPendingAdd } = useMutation({
    mutationFn: async () => {
      const response = await axiosInstanceToken.post("/v1/api/kabar-berita", {
        image: images,
  ...
  const { mutate: mutateEdit, isPending: isPendingEdit } = useMutation({
    mutationFn: async () => {
      const response = await axiosInstanceToken.put(
        `/v1/api/kabar-berita/${selectedItem?.id}`,
        {
          image: images,
          title: form.getValues("title"),
          content: form.getValues("content"),
